    ...
    public void notifyIterationEnds(IterationEndsEvent event) {
        ...
        for (String mode : drtModes) {
            ...
            if (error > 0) {
                ...
                log.info("Iteration {}: Mode {} - actualRate={:.1f}%, error={:.3f}, penalty: {:.3f} → {:.3f}",
                        iteration, mode, actualRate * 100, error, previousPenalty, newPenalty);
            } else {
                log.info("Iteration {}: Mode {} - actualRate={:.1f}%, penalty unchanged: {:.3f}",
                        iteration, mode, actualRate * 100, previousPenalty);
            }
        }
    }
