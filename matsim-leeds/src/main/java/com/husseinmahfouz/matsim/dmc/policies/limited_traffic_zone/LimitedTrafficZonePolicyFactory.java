...
import com.husseinmahfouz.matsim.dmc.policies.routing.FactorRoutingPenalty;
...
public class LimitedTrafficZonePolicyFactory implements PolicyFactory {
	private static final Logger logger =
			LogManager.getLogger(LimitedTrafficZonePolicyFactory.class);
	...
	private Policy createPolicy(LimitedTrafficZoneConfigGroup ltzConfig,
		...
		logger.info("Creating policy " + ltzConfig.policyName + " of type "
				+ LimitedTrafficZonePolicyFactory.POLICY_NAME);

		if (!ltzConfig.perimetersPath.isEmpty() && !ltzConfig.linkListPath.isEmpty()) {
			throw new IllegalStateException(
					"Only one of perimetersPath and linkListPath can be set for policy "
							+ ltzConfig.policyName);
		}
		...
		if (!ltzConfig.perimetersPath.isEmpty()) {
			...
			linkIds = PolicyLinkFinder.create(new File(ConfigGroup
					.getInputFileURL(config.getContext(), ltzConfig.perimetersPath).getPath()))
			...
			linkIds = loadLinkList(ConfigGroup
					.getInputFileURL(config.getContext(), ltzConfig.linkListPath).getPath(),
					network, ltzConfig.policyName);

			logger.info("  Affected links: " + linkIds.size());
		} else {
			throw new IllegalStateException(
					"One of perimetersPath and linkListPath must be set for policy "
							+ ltzConfig.policyName);
		}

		return new DefaultPolicy(new FactorRoutingPenalty(linkIds, insideFactor, personFilter),
				null);
	}
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (!line.isEmpty()) {
					...
					if (link == null) {
						throw new IllegalStateException("Link list of policy " + policy
								+ " contains link " + line + " which is not included in network");
					}
