...
read_and_process <- function(scenario, fleet_size) {
  ...
  files <- list.files(
    path = dir_path,
    pattern = "^output_occupancy_time_profiles_.*\\.txt$",
    full.names = TRUE
  )
  ...
  data_list <- lapply(files, function(file) {
    ...
    file_suffix <- gsub(
      "^output_occupancy_time_profiles_|\\.txt$",
      "",
      basename(file)
    )
...
drt_occupancy_time <- purrr::pmap_dfr(
  combinations,
...
drt_occupancy_time_l <- drt_occupancy_time %>%
  pivot_longer(
    cols = -c(time, scenario, fleet_size, operator_id),
    names_to = "Category",
    values_to = "Vehicles"
  ) %>%
...
drt_occupancy_time_l = drt_occupancy_time_l %>%
  mutate(
    operator_id = case_when(
      operator_id == "drt" ~ "drtAll",
      .default = operator_id
    )
  )


drt_occupancy_time_l$Category <- forcats::fct_relevel(
  drt_occupancy_time_l$Category,
  "STAY",
  "RELOCATE",
  "0 pax",
  "1 pax",
...
# --- Line plot
ggplot(
  drt_occupancy_time_l, #%>%
  #filter(!(Category %in% c("STAY", "RELOCATE")))
  aes(x = time, y = Vehicles_Share, color = Category)
) +
  geom_line() +
  scale_x_datetime(
    date_breaks = "4 hour", # Adjust to show more labels every hour
    date_labels = "%H:%M",
    minor_breaks = "1 hour"
  ) + # Show only hour and minute
  labs(
    title = "Vehicle occupancy by time of day",
    x = "",
...
ggsave(
  paste0(plots_dir, "vehicle_occupancy_line_facet_scenario_and_fleet_size.png"),
  bg = "white",
  width = 7
)
...
ggsave(
  paste0(plots_dir, "vehicle_occupancy_area_facet_scenario_and_fleet_size.png"),
  bg = "white",
  width = 7
)
...
drt_vkm_time = drt_vkm %>%
  mutate(
    departure_time_h = round(departure_time / 3600),
...
ggplot(drt_km_time, aes(x = arrival_time_h, y = distance_km, fill = metric)) +
  ...
  labs(
    title = "Vehicle and passenger km throughout the day",
    x = "",
    y = "Distance (km)",
    fill = ""
  ) +
...
ggsave(
  paste0(plots_dir, "vkm_pkm_bar_facet_scenario_and_fleet_size.png"),
  bg = "white",
  width = 7
)
...
ggplot(
  ...
  geom_line() +
  labs(
    title = "Cumulative vehicle and passenger km throughout the day",
    x = "",
    y = "Distance (km)",
    color = ""
  ) +
...
ggsave(
  paste0(
    plots_dir,
    "vkm_pkm_cumulative_line_facet_scenario_and_fleet_size.png"
  ),
  bg = "white",
  width = 7
)
...
drt_km_time_load_factor = drt_km_time %>%
  ...
  mutate(
    load_factor = distance_km_pkm / distance_km_vkm,
    load_factor_cum = distance_km_cum_pkm / distance_km_cum_vkm
  )

# plot
ggplot(
  drt_km_time_load_factor %>%
...
ggplot(
  ...
  geom_line(lwd = 2) +
  labs(
    title = "Ratio of occupied km (Evolution over the day)",
...
ggsave(
  paste0(
    plots_dir,
    "load_factor_cumulative_line_facet_scenario_and_fleet_size.png"
  ),
  bg = "white",
  width = 7
)
...
drt_pkm_passengers = drt_km_time %>%
  ...
  left_join(
    drt_passengers,
    by = c("arrival_time_h", "operator_id", "scenario", "fleet_size")
  ) %>%
  ...
  mutate(
    passenger_distance_per_trip = round(distance_km_pkm / passengers, 2),
    passenger_distance_per_trip_cum = round(
      distance_km_cum_pkm / passengers_cum,
      2
    ),
    vehicle_distance_per_trip = round(distance_km_vkm / passengers, 2),
    vehicle_distance_per_trip_cum = round(
      distance_km_cum_vkm / passengers_cum,
      2
    )
  )
...
drt_pkm_passengers = drt_pkm_passengers %>%
  pivot_longer(
    cols = c(
      #distance_km_pkm, distance_km_cum_pkm,
      #distance_km_vkm, distance_km_cum_vkm,
      #passengers, passengers_cum,
      passenger_distance_per_trip,
      passenger_distance_per_trip_cum,
      vehicle_distance_per_trip,
      vehicle_distance_per_trip_cum
    ),
    names_to = "metric"
  )
...
drt_pkm_passengers = drt_pkm_passengers %>%
  mutate(
    category = case_when(
      stringr::str_starts(metric, "passenger_") ~ "passenger",
      stringr::str_starts(metric, "vehicle_") ~ "vehicle"
    ),
    cumulative = case_when(
      stringr::str_ends(metric, "_cum") ~ "yes",
      TRUE ~ "no"
    )
  )
...
ggplot(
  ...
  guides(
    colour = guide_legend(nrow = 2), # Force a single-row legend
...
ggsave(
  paste0(
    plots_dir,
    "distance_per_passenger_trip_line_facet_scenario_and_fleet_size.png"
  ),
  bg = "white",
  width = 7
)
...
ggsave(
  paste0(
    plots_dir,
    "distance_per_passenger_trip_bar_facet_scenario_and_fleet_size.png"
  ),
  bg = "white",
  width = 7
)
...
ggplot(
  drt_vkm_occupancy,
  aes(
    x = factor(number_of_passengers),
    y = distance_km,
    fill = factor(number_of_passengers)
  )
) +
  geom_col() +
  facet_grid(vars(fleet_size), vars(operator_id)) +
  labs(
    title = "Total distance travelled by passenger load",
...
ggsave(
  paste0(
    plots_dir,
    "distance_by_passenger_count_bar_facet_scenario_and_fleet_size.png"
  ),
  bg = "white",
  width = 7
)
...
drt_km_time_table = drt_km_time %>%
  ...
  mutate(
    pkm_per_vehicle = round(pkm / fleet_size),
...
drt_pkm_passengers_table = drt_pkm_passengers %>%
  ...
  group_by(operator_id, scenario, fleet_size, metric) %>%
  summarise(
    average_distance_per_passenger_trip = round(mean(value, na.rm = TRUE), 1)
  ) %>%
  ungroup() %>%
  pivot_wider(
    names_from = metric,
    values_from = average_distance_per_passenger_trip
  )


drt_table = drt_km_time_table %>%
  left_join(
    drt_pkm_passengers_table,
    by = c("operator_id", "scenario", "fleet_size")
  )


# ----------------- NEW: Fleet Efficiency from MATSim Stats -----------------
# Reviewer request: Use reciprocal of "l_det" from drt_vehicle_stats
# # Based on: https://www.sciencedirect.com/science/article/pii/S2667091725000755?via%3Dihub

drt_efficiency <- purrr::pmap_dfr(combinations, function(scenario, fleet_size) {
  dir_path <- paste0(
    "../scenarios/fleet_sizing/",
    scenario,
    "/",
  ...
  # Find the stats file (handles drt.csv, drtNE.csv, etc.)
  files <- list.files(
    dir_path,
    pattern = "^drt_vehicle_stats_.*\\.csv$",
    full.names = TRUE
  )
  ...
  lapply(files, function(f) {
    # Read stats
    stats_df <- read_delim(f, delim = ";", show_col_types = FALSE)

    # Get suffix for operator matching
    suffix <- gsub("^drt_vehicle_stats_|\\.csv$", "", basename(f))

    # Get value from LAST iteration row
    last_row <- tail(stats_df, 1)

    tibble(
      scenario = scenario,
      fleet_size = fleet_size,
      operator_id = suffix,
      l_det = last_row$l_det
    )
  }) %>%
    bind_rows()
})

# Normalize operator names to match main table
drt_efficiency <- drt_efficiency %>%
  mutate(
    operator_id = case_when(
      operator_id == "drt" ~ "drtAll",
      TRUE ~ operator_id
...
# Join and Calculate Efficiency
drt_table <- drt_table %>%
  left_join(drt_efficiency, by = c("operator_id", "scenario", "fleet_size")) %>%
  mutate(
    fleet_efficiency = round(1 / l_det, 2)
  )
