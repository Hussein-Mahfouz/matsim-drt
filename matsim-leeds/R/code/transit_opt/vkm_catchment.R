...
vkm_catchment <- function(
  ...
  if (level == "all" && access == "all") {
    ...
    # Person filtering needs access to full person set within access subset
    if (level == "person") {
      # Logic moved below to share code
      trips_access <- filter_by_person(trips, trips_access)
    }
...
# Helper for person level filtering
filter_by_person <- function(all_trips, filtered_trips) {
    # message("Filtering by person...")
    total_trips_per_person <- all_trips |> count(person_id, name = "total_trips")
    filtered_trips_per_person <- filtered_trips |>
    ...
    filtered_trips |> filter(person_id %in% persons_all_in)
}
...
pt_vkm_from_gtfs <- function(solution_dir, boundary_sf) {
  ...
  stop_times <- read_csv(file.path(temp_gtfs_dir, "stop_times.txt"), show_col_types = FALSE)
  gtfs_stops <- read_csv(file.path(temp_gtfs_dir, "stops.txt"), show_col_types = FALSE) |>
  ...
  stop_times |>
    ...
    summarize(total_distance_m = sum(distance_m, na.rm = TRUE), .groups = "drop") |>
    summarize(total_distance_km = sum(total_distance_m) / 1000) |>
    mutate(mode = "pt", share = 100, .before = everything())
}
...
impute_stuck_trips_vkm <- function(solution_trips, base_trips) {
  ...
  # 4. Filter simulation data
  # Keep only valid people from solution
